...
环境变量（调度相关）
- config.work.max_running_tasks: Worker 注册时上报最大并发任务数，默认 1
- config.worker.select_strategy: Worker 选择策略，"least-load"（默认）或 "rr"
- config.dispatch.max_retries: Master 派发到 Worker 的重试次数，默认 2
