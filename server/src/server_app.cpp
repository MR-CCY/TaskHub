...
// Forward declaration for seed function
#include "router.h"
...
int ServerApp::run() {
  ...
  if (Config::instance().get("work.is_work", false)) {
    ...
    // - advertise_host: the address reachable by master/other workers (e.g.
    // docker service name)
    const std::string legacyWorkerHost =
        Config::instance().get<std::string>("work.worker_host", "0.0.0.0");
    const std::string bindHost =
        Config::instance().get<std::string>("work.bind_host", legacyWorkerHost);
    const std::string advertiseHost =
        Config::instance().get<std::string>("work.advertise_host", bindHost);

    m_host = bindHost;
    m_port = Config::instance().get("work.worker_port", 8083);

    // master_host is the address the worker uses to connect to master.
    // NOTE: "0.0.0.0" is only meaningful for binding/listening; it is not a
    // connectable destination.
    std::string masterHost =
        Config::instance().get<std::string>("work.master_host", "127.0.0.1");

    int masterPort = Config::instance().get("work.master_port", 8082);
    std::string workerId =
        Config::instance().get<std::string>("work.worker_id", "worker-1");
    int intervalMs = Config::instance().get("work.heartbeat_interval_ms", 2000);
    std::vector<std::string> queues =
        Config::instance().get<std::vector<std::string>>("work.queues");
    std::vector<std::string> labels =
        Config::instance().get<std::vector<std::string>>("work.labels");
    auto test = []() -> int { return 0; };

    g_heartbeat = std::make_unique<worker::WorkerHeartbeatClient>();
    g_heartbeat->start(masterHost, masterPort, workerId, advertiseHost, m_port,
                       queues, labels, test,
  ...
  worker::ServerWorkerRegistry::instance().startSweeper(
      std::chrono::seconds(5), std::chrono::seconds(60));
  // 12. 启动监听（阻塞）
...
void ServerApp::init_config() {
  ...
  if (!loaded) {
    // 3) 源码默认配置
    loaded =
        cfg.load((cwd / "server" / "config" / "default_config.json").string());
  }
  if (!loaded) {
    Logger::warn(
        "No config file found in fallback paths, using built-in defaults");
  }
...
void ServerApp::init_logger() {
  ...
  Logger::info(
      ...
      (logPath.empty() ? " (console-only)" : (" (file=" + logPath + ")")));
}

void ServerApp::init_http_server() {
  m_server = std::make_unique<httplib::Server>();
}
void ServerApp::setup_routes() { Router::setup_routes(*m_server); }
void ServerApp::init_db() {
  ...
  const std::string migrations_dir = Config::instance().get<std::string>(
