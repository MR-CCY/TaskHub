...
void MainWindow::setupUserBar() {
  ...
  // 从 AppContext 读取登录用户名（LoginDialog 登录后已写入）
  const QString savedUser = AppContext::instance().username();
  userBar_->setUsername(savedUser.isEmpty() ? tr("-") : savedUser);
...
void MainWindow::wireSignals() {
  ...
  connect(
      api_, &ApiClient::requestFailed, this,
      [this](const QString &apiName, int st, const QString &msg) {
        console_->appendError(
            QString("HTTP %1 failed (%2): %3").arg(apiName).arg(st).arg(msg));
      });
  ...
  connect(api_, &ApiClient::loginOk, this,
          [this](const QString &token, const QString &username) {
            onLoginOk(token, username);
          });

  connect(api_, &ApiClient::loginFailed, this,
          [this](int st, const QString &msg) {
            console_->appendError(
                QString("login failed (%1): %2").arg(st).arg(msg));
          });

  connect(ws_, &WsClient::connected, this,
          [this]() { console_->appendInfo("WS connected, sent token"); });
  ...
  connect(ws_, &WsClient::messageReceived, this,
          [this](const QJsonObject &obj) {
            ...
            console_->appendEvent(
                QString("WS msg: %1")
                    .arg(QString::fromUtf8(
                        QJsonDocument(obj).toJson(QJsonDocument::Compact))));
          });

  auto subscribeTasks = [this](const QString &tag, const QString &runId,
                               const QJsonArray &taskIds) {
    ...
    for (const auto &item : taskIds) {
      ...
      console_->appendInfo(
          QString("subscribed logs/events for task %1 run %2").arg(tid, runId));
    }
  };

  connect(api_, &ApiClient::runDagAsyncOk, this,
          [subscribeTasks](const QString &runId, const QJsonArray &taskIds) {
            subscribeTasks(QStringLiteral("dag run_async"), runId, taskIds);
          });
  connect(api_, &ApiClient::runTemplateAsyncOk, this,
          [subscribeTasks](const QString &runId, const QJsonArray &taskIds) {
            subscribeTasks(QStringLiteral("template run_async"), runId,
                           taskIds);
          });
  ...
  connect(ws_, &WsClient::closed, this,
          [this]() { console_->appendError("WS closed"); });
}
...
bool MainWindow::eventFilter(QObject *obj, QEvent *event) { return false; }
