...
NodeInspectorRuntimeWidget::NodeInspectorRuntimeWidget(QWidget *parent)
  ...
  runtimeStdoutEdit_ = new QPlainTextEdit(this);
  runtimeStdoutEdit_->setReadOnly(true);
  runtimeStdoutEdit_->setFixedHeight(80);
  runtimeStdoutEdit_->setPlaceholderText("-");

  runtimeStderrEdit_ = new QPlainTextEdit(this);
  runtimeStderrEdit_->setReadOnly(true);
  runtimeStderrEdit_->setFixedHeight(80);
  runtimeStderrEdit_->setPlaceholderText("-");
  ...
  form->addRow(tr("Stdout"), runtimeStdoutEdit_);
  form->addRow(tr("Stderr"), runtimeStderrEdit_);
...
void NodeInspectorRuntimeWidget::setRuntimeValues(const QVariantMap &obj) {
  ...
  runtimeAttemptLabel_->setText(
      QString("%1 / %2").arg(attempt).arg(maxAttempts));
  runtimeWorkerLabel_->setText(worker);
  runtimeMessageLabel_->setText(msg);
  runtimeStdoutEdit_->setPlainText(stdout);
  runtimeStderrEdit_->setPlainText(stderr);
  if (startTime > 0) {
    QDateTime startDateTime = QDateTime::fromMSecsSinceEpoch(startTime);
    runtimeStartTimeLabel_->setText(
        startDateTime.toString("yyyy-MM-dd HH:mm:ss.zzz"));
  } else {
    runtimeStartTimeLabel_->setText("-");
  }
  ...
  if (endTime > 0) {
    ...
    runtimeEndTimeLabel_->setText(
        endDateTime.toString("yyyy-MM-dd HH:mm:ss.zzz"));
  } else {
    runtimeEndTimeLabel_->setText("-");
  }
}