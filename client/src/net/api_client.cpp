...
void ApiClient::getHealth() { getJson("health", "/api/health"); }

void ApiClient::getInfo() { getJson("info", "/api/info"); }

void ApiClient::runDagAsync(const QJsonObject &body) {
  postJson("runDagAsync", "/api/dag/run_async", body);
}

void ApiClient::listTemplates() { getJson("templates", "/templates"); }

void ApiClient::runTemplateAsync(const QString &templateId,
...
void ApiClient::listTaskTemplates(const QString &execType) {
  QString path = "/api/task_templates";
  if (!execType.isEmpty()) {
    path += "?type=" + execType;
  }
  getJson("taskTemplates", path);
}

void ApiClient::getTaskTemplate(const QString &templateId) {
  if (templateId.isEmpty())
    return;
  getJson("taskTemplate:" + templateId, "/api/task_templates/" + templateId);
}

void ApiClient::getDagRuns(const QString &runId, const QString &name,
                           const QString &source, qint64 startTsMs,
                           qint64 endTsMs, int limit) {
  QUrlQuery q;
  if (!runId.isEmpty())
    q.addQueryItem("run_id", runId);
  if (!name.isEmpty())
    q.addQueryItem("name", name);
  if (!source.isEmpty())
    q.addQueryItem("source", source);
  if (startTsMs > 0)
    q.addQueryItem("start_ts_ms", QString::number(startTsMs));
  if (endTsMs > 0)
    q.addQueryItem("end_ts_ms", QString::number(endTsMs));
  if (limit > 0)
    q.addQueryItem("limit", QString::number(limit));
  const QString path =
      "/api/dag/runs" +
      (q.isEmpty() ? QString() : QString("?" + q.toString(QUrl::FullyEncoded)));
  getJson("dagRuns", path);
}

void ApiClient::getTaskRuns(const QString &runId, const QString &name,
                            int limit) {
  QUrlQuery q;
  if (!runId.isEmpty())
    q.addQueryItem("run_id", runId);
...
void ApiClient::getRemoteTaskRuns(const QString &runId,
                                  const QString &remotePath, int limit) {
  QUrlQuery q;
  if (!runId.isEmpty())
    q.addQueryItem("run_id", runId);
  if (!remotePath.isEmpty())
    q.addQueryItem("remote_path", remotePath);
  if (limit > 0)
    q.addQueryItem("limit", QString::number(limit));
  const QString path =
      "/api/workers/proxy/dag/task_runs" +
      (q.isEmpty() ? QString() : QString("?" + q.toString(QUrl::FullyEncoded)));
...
void ApiClient::getRemoteDagEvents(const QString &runId,
                                   const QString &remotePath, qint64 startTsMs,
                                   int limit) {
  QUrlQuery q;
  if (!runId.isEmpty())
    q.addQueryItem("run_id", runId);
  ...
  const QString path =
      "/api/workers/proxy/dag/events" +
      (q.isEmpty() ? QString() : QString("?" + q.toString(QUrl::FullyEncoded)));
  getJson("remoteDagEvents:" + remotePath, path);
}

void ApiClient::listCronJobs() { getJson("cronJobs", "/api/cron/jobs"); }
...
void ApiClient::getWorkers() { getJson("workers", "/api/workers"); }
...
void ApiClient::getJson(const QString &apiName, const QString &path) {
  ...
  connect(reply, &QNetworkReply::errorOccurred, this,
          [this, apiName](QNetworkReply::NetworkError) {
...
void ApiClient::postJson(const QString &apiName, const QString &path,
...
void ApiClient::deleteJson(const QString &apiName, const QString &path,
                           const QString &idContext) {
  if (unauthorized_) {
    return;
  }
  if (baseUrl_.isEmpty()) {
...
bool ApiClient::parseJsonObject(const QByteArray &bytes, QJsonObject &out,
                                QString &err) {
  QJsonParseError pe;
...
ApiClient::ParsedEnvelope ApiClient::parseEnvelope(const QJsonObject &root) {
  ...
  if (root.contains("ok")) {
    ...
    env.message = !err.isEmpty()
                      ? err
                      : (!msg.isEmpty() ? msg : (ok ? "ok" : "request failed"));
...
void ApiClient::onRawJson(const QString &apiName, const QJsonObject &root) {
  ...
  if (!env.ok) {
    ...
    if (apiName == "login")
      emit loginFailed(0, env.message); // status 0 or generic?
    else
      emit requestFailed(apiName, 0, env.message);
    ...
    // If we want httpStatus, we need to change rawJson signature or embed it in
    // root (hacky). Or we assume logic error has 0 status. Most logic errors
    // (env.ok == false) are application level errors (code != 0), even if HTTP
    // 200. So status 0 or 200 is acceptable approximation, or we extend
    // rawJson. Given constraint "internal signal", we can change rawJson
    // signature easily!
    return;
  }
  ...
  if (apiName == "health") {
    if (env.data.isObject())
      emit healthOk(env.data.toObject());
    else
      emit requestFailed(apiName, 0, "health data is not an object");
  } else if (apiName == "info") {
    if (env.data.isObject())
      emit infoOk(env.data.toObject());
    else
      emit requestFailed(apiName, 0, "info data is not an object");
  ...
  } else if (apiName == "taskTemplates") {
    QJsonArray templates;
    if (env.data.isObject()) {
      templates = env.data.toObject().value("templates").toArray();
    } else if (env.data.isArray()) {
      templates = env.data.toArray();
    }
    emit taskTemplatesOk(templates);
  } else if (apiName.startsWith("taskTemplate:")) {
    if (env.data.isObject()) {
      const QJsonObject templateData =
          env.data.toObject().value("template").toObject();
      emit taskTemplateOk(templateData);
    } else {
      emit requestFailed(apiName, 0, "task template data is not an object");
    }
    ...
    if (env.data.isObject()) {
      emit remoteTaskRunsOk(rPath,
                            env.data.toObject().value("items").toArray());
    } else {
