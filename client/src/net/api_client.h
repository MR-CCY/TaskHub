...
public:
  ...
  // Task templates (for Shell/HTTP/Local nodes)
  void listTaskTemplates(const QString &execType = QString());
  void getTaskTemplate(const QString &templateId);
  void getDagRuns(const QString &runId = QString(),
                  const QString &name = QString(),
                  const QString &source = QString(), qint64 startTsMs = 0,
                  qint64 endTsMs = 0, int limit = 100);
  void getTaskRuns(const QString &runId = QString(),
                   const QString &name = QString(), int limit = 200);
  // Proxy for remote task runs
  void getRemoteTaskRuns(const QString &runId, const QString &remotePath,
                         int limit = 200);

  void getDagEvents(const QString &runId = QString(),
                    const QString &taskId = QString(),
                    const QString &type = QString(),
                    const QString &event = QString(), qint64 startTsMs = 0,
                    qint64 endTsMs = 0, int limit = 200);
  // Proxy for remote dag events
  void getRemoteDagEvents(const QString &runId, const QString &remotePath,
                          qint64 startTsMs = 0, int limit = 200);
  void listCronJobs();
...
signals:
  ...
  void taskTemplatesOk(const QJsonArray &templates);
  void taskTemplateOk(const QJsonObject &templateData);
  ...
  // generic
  void requestFailed(const QString &apiName, int httpStatus,
                     const QString &message);
  void rawJson(const QString &apiName,
               const QJsonObject &obj); // optional: for ConsoleDock
...
private:
  ...
  void postJson(const QString &apiName, const QString &path,
                const QJsonObject &body);
  void deleteJson(const QString &apiName, const QString &path,
                  const QString &idContext = QString());

  static bool parseJsonObject(const QByteArray &bytes, QJsonObject &out,
                              QString &err);
