...
- Qt6 客户端采用 STOC（Scene-Task-Operator-Command）分层交互架构
- 客户端核心编辑动作命令化（`QUndoStack`），支持撤销/重做

## 服务端技术亮点（简要）
- 执行引擎采用策略模式 + 注册表：`Local/Shell/HttpCall/Script/Remote/Dag/Template` 统一调度入口，新增执行类型成本低。
- 分布式 Worker 具备完整生命周期管理：注册、心跳、存活判定、后台清理（含空闲休眠唤醒）。
- 调度具备容量感知：`max_running_tasks` + `full` 过滤，避免把任务派发到已满 Worker。
- 选路策略可配置可扩展：`least-load` 与 `round-robin`，并支持按队列维度选路。
- 远程派发具备工程级可靠性：ACK（HTTP 2xx）判定、超时/5xx 重试、失败节点冷却与 failover。
- DAG 引擎按拓扑入度驱动，支持 `max_parallel` 并行控制与 `FailFast/SkipDownstream` 失败策略。
- DAG 线程池支持优先级队列与动态扩容，降低嵌套 DAG 场景下的线程饥饿风险。
- Cron 统一支持 `SingleTask/Dag/Template` 三类目标触发，执行与持久化链路一致。
- 可观测性闭环：结构化日志 + WebSocket 实时推送 + `task_event` 落库。
- 运行数据三层持久化：`dag_run/task_run/task_event`，支持运行追踪、审计与排障回放。
...
## 客户端技术亮点（简要）
- 统一事件入口：`CanvasView::viewportEvent` 收口输入后交由 `TaskManager` 分发。
- Task 栈式交互：基于 `level` 的任务分层，临时任务退出后可恢复主任务。
- 命令化编辑链路：创建/删除/移动/连线/属性修改走 `QUndoCommand + QUndoStack`。
- 批量编辑事务化：通过 `beginMacro/endMacro` 将多次改动合并为一次撤销单元。
- 节点扩展机制：`NodeItemFactory` 统一创建 `Shell/Http/Local/Remote/Dag/Template` 节点。
- 嵌套 DAG 支持：`DagLoader/DagSerializer` 递归处理容器节点子图导入导出。
- 连线约束前置：`LineItemFactory::canConnect` 在编辑阶段拦截非法连接。
- 运行监控双通道：HTTP 轮询 + WS 推送结合，兼顾完整性与实时性。
- 网络层归一化：`ApiClient` 统一解析响应 envelope，并集中处理 401 会话失效。
...
## 备注
- 默认 WebSocket 监听端口为 `8090`（Docker 端口映射见上文“端口映射”）
